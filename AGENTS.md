...
Then create `mobile/.env` with your Codex API key:
```
...
- [ ] Paste real Codex API key into `mobile/.env`
...
Think Duolingo gamification meets Google Calendar scheduling, with Codex as a practice coach.
...
| AI | Codex API (Haiku 4.5) via direct fetch in `mobile/src/utils/ai.js` |
...
mobile/
  ...
  src/
    ...
    utils/
      ai.js                       # All Codex API calls — key loaded from .env
  ...
  .env                            # GITIGNORED — paste Codex API key here
...
- **Codex API key** — loaded from `mobile/.env` as `EXPO_PUBLIC_ANTHROPIC_KEY`. Never hardcode in source. File is gitignored.
...
- **Mood logs** — three types: `quick` (post-session, ~60s), `deep` (weekly reflection, 5-6 Codex questions), `pre_session` (before timer starts, mood + Codex intention question).
...
- **Mood check-in should feel like a conversation**, not a form. Codex prompts questions one at a time — "how are you feeling?", emoji scale, then "name two things you're grateful for", "anything frustrating you?", "what are your goals this week?" — not a static survey.
...
- **Google Calendar context** makes recommendations much smarter — if Codex sees "AP Biology exam tomorrow", it recommends a short targeted session instead of a full drill. This is why OAuth matters.
- **Notebook** should be closer to Google Docs than a text field. Rich text, hyperlinks. AI generates a table of contents. Toggle: "let AI read my notebook" → Codex uses it as extra coaching context.
- **AI Chat screen** — student types "I've been playing these big band charts, recommend similar ones" and Codex knows their full history (tasks, notes, ensemble, mood). Not a generic chatbot — fully context-loaded.
...
- **API cost**: using Codex Haiku 4.5 — ~$0.80/1M input tokens, plenty for demo on $5 credit. Could upgrade just `getPracticeRecommendation` to Sonnet if output feels generic (one-line change in ai.js).
...
- Pre-session check-in: mood + Codex intention question before timer starts
- Tasks: filter tabs, manual create, Smart Add (Codex structures natural language), Task Detail, readiness bar
- Practice notes (per-task AI memory), AI coaching tip, smart notifications (Codex picks message + send time)
...
- Mindful Practice layer: pre-session check-in (mood + Codex intention), post-session quick reflection, weekly deep check-in (Codex generates personalised 5-6 question survey)
...
- **Notebook**: entry list + search, autosave editor, tag chips (Reflection/Lesson Note/Music Writing/Technique/Repertoire/General), AI Table of Contents, "Let Codex read" toggle, post-session reflection prompt
...
- Added `callClaudeWithHistory()` internal helper for multi-turn Codex calls with system prompt
...
- Updated AGENTS.md with first-time setup instructions for VS Code
- Codex API key now loaded from `mobile/.env` (gitignored) via `EXPO_PUBLIC_ANTHROPIC_KEY`

### Session: continue-app-development (continued)
- **Built Notebook feature** — fully integrated:
  - `Notebook.jsx` — entry list, search, "Let Codex read" toggle, AI Table of Contents button
...
- Built `PreSessionCheckIn` — mood + Codex intention question shown before timer starts
...
- Created `TODO.md` and `AGENTS.md` for persistent context
- Agreed: AGENTS.md gets updated every session with decisions, ideas, and progress
...
- Decided on two mood modes: quick (every session, ~60s) and deep (weekly, Codex-generated questions)
...
- **Active branch**: `Codex/continue-app-development-tBZDj`
